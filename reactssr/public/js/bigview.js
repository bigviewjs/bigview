...
var Bigview = function () {
  ...
  this.on('pageletArrive', function (payload) {
    ...
    if (typeof __webpack_modules__ !== "object") {
      //react未ready，放到bigview.js的cache里
      //TODO
      this.cache.push(payload.domid)
    }else{
      //TODO ReactDOM.hydrate(<a/>, document.getElementById('app'));
      console.log('TODO ReactDOM.hydrate(<a/>, document.getElementById(')
      console.log('bigview#call biglet_'+payload.domid+'()')
      window['biglet_'+payload.domid]()
    }
  })
  ...
  this.handlePayload = function (payload) {
    ...
    if (payload.domid && !payload.error && typeof document === 'object') {
      self.replaceHtml(payload.domid, document.getElementById(payload.domid + '-code').innerHTML, payload.attr)
    }
    if (payload.js) {
